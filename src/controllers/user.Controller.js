...
const addProduct = async (req, res) => {
  ...
  const getRecord = await getAllrecordFromRedis("products");
  const redisData = [];
  if (getRecord && getRecord.length) {
    getRecord.push(product.dataValues);
    redisData.push(...getRecord);
  } else {
    redisData.push(product.dataValues);
  }
  await insertRedisRecord("products", redisData);
...
const getAllProducts = async (req, res) => {
  ...
  let result;
  if (payload) {
    const res = await getAllrecordFromRedis("products");
    result = { count: res.length, rows: res };
  } else {
    result = await productSrvice.queryFiles(payload);
  }
  res.send(result);
};
...
const getOneProduct = async (req, res) => {
  ...
  const getProduct = await getOneRecordFromRedisCatch("products", id);
  if (getProduct) {
...
const updateProduct = async (req, res) => {
  ...
  const product = await productSrvice.updateProduct(req.body, {
    where: { id: id },
  });
  console.log('product', product);
  let result;
  if (product) {
    result = await updateRedisRecord("products", product.dataValues, product.dataValues.id);
  }
...
const deleteProduct = async (req, res) => {
  ...
  await deleteRedisRecord("products", getRecord, parseInt(id));
